...
import { CheckIcon, DrumIcon, FilterIcon, Guitar, LucidePause, Music2, PauseCircle, PauseIcon, PauseOctagon, Piano, Play, PlayCircle, PlayIcon, Plus, SortAsc, SortAscIcon, SortDesc, SortDescIcon } from 'lucide-react'
...
function MusicComponent() {
  ...
  return (
    <div className='flex scroll-smooth h-full overflow-y-auto rounded-2xl w-[90%] sm:w-full border-1 py-2 flex-col items-center px-4 justify-start  bg-[var(--bgColor)]/50 backdrop-blur-md  text-[var(--text-color)] gap-2'>
          ...
          <div className='flex gap-2 w-full justify-evenly items-start'>
            <div className='relative'>
              ...
              > <div className='flex gap-2 justify-center items-center'>
                {aToZActive ? <SortAsc size={25} strokeWidth={1}/> : <SortDesc size={25} strokeWidth={1}/>}
                  <p>Sort</p>
                </div>
              <p className='border-1 px-2 text-[8px] border-[var(--borderColor)] rounded-md'>
                {aToZActive ? 'A-Z' : 'Z-A'}
              </p>
              </button>
              {showSortPopup && 
            ...
            <div className='relative'>
              ...
              {showFilterPopup && 
              <div className='absolute z-40 py-6 px-12 rounded-2xl -right-[68%] top-[110%] bg-[var(--wrapperColor)] text-[var(--textColor)] backdrop-blur-sm flex flex-col justify-center items-center'>
                  <button
                  onClick={() => {
                  ...
                  className={`mb-2 border-1 w-full flex justify-center items-center gap-2 border-[var(--borderColor)] cursor-pointer active:scale-75 py-2 px-21 rounded-md ${allButtonActive ? 'bg-[var(--textColor)] text-[var(--bgColor)] font-semibold text-xl' : ''}`}
                  ...
                  className={`mb-2 border-1 flex justify-center items-center gap-2 border-[var(--borderColor)] ${bollywoodButtonActive ? 'bg-[var(--textColor)] text-[var(--bgColor)] font-semibold text-xl' : ''} cursor-pointer active:scale-75 py-2 w-full rounded-md`}
                  ...
                  className={`mb-2 flex w-full justify-center items-center gap-2 border-1 border-[var(--borderColor)] ${rockButtonActive ? 'bg-[var(--textColor)] text-[var(--bgColor)] font-semibold text-xl' : ''} cursor-pointer active:scale-75 py-2  rounded-md`}
            ...
            {bollywoodButtonActive ? <div>
               {musicData.filter((item) => item.genre.startsWith('Bollywood') || item.genre === 'Bollywood').map((item, index) => (
            <span key={index} className='flex w-full h-fit items-center justify-between gap-2 px-2 border-1 border-[var(--borderColor)] bg-[var(--bgColor)]/15 rounded-xl mb-3'>
               <div className='flex items-center gap-2 px-2'>
                <Image src={item.image} className='rounded-sm w-8 h-8' width={40} height={40} alt="music artist" />
            ...
            {rockButtonActive ? <div>
               {musicData.filter((item) => !item.genre.startsWith('Bollywood')).map((item, index) => (
            <span key={index} className='flex w-full h-fit items-center justify-between gap-2 px-2 border-1 border-[var(--borderColor)] bg-[var(--bgColor)]/15 rounded-xl mb-3'>
               <div className='flex items-center gap-2 px-2'>
                <Image src={item.image} className='rounded-sm w-8 h-8' width={40} height={40} alt="music artist" />
            ...
            {showSortPopup || showFilterPopup ? (
              ...
              className='fixed w-screen h-screen inset-0 z-30 bg-black/70'
              ></div>
            ) : null}
