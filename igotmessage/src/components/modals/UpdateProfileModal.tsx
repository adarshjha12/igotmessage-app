"use client";

import { Dialog, Transition, TransitionChild } from "@headlessui/react";
import { Fragment, useEffect, useState } from "react";
import { CheckCircle2, XCircle, UploadCloud } from "lucide-react";
import { useAppDispatch, useAppSelector } from "@/store/hooks";
import { setShowProfileUpdateModal } from "@/features/authSlice";

export default function ProfileUpdateModal() {
  const dispatch = useAppDispatch();
  const updateStatus = useAppSelector((state) => state.auth.user.updateProfileStatus);
  const showProfileUpdateModal = useAppSelector(
    (state) => state.auth.user.showProfileUpdateModal
  );
  const [progress, setProgress] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      progress < 90 && setProgress((prev) => prev + 10);
    }, 500);
    return () => clearInterval(interval);
  }, [progress]);

  const getStatusUI = () => {
    switch (updateStatus) {
      case "loading":
        return {
          icon: null,
          title: "updating profile...",
          message: "Hold tight! Your profile is updating.",
        };
      case "succeeded":
        return {
          icon: <CheckCircle2 className="h-10 w-10 text-green-500" />,
          title: "Updated Successfully!",
          message: "Your profile is now updated.",
        };
      case "failed":
        return {
          icon: <XCircle className="h-10 w-10 text-red-500" />,
          title: "update Failed!",
          message: "Something went wrong. Please try again.",
        };
    }
  };

const { icon, title, message } = getStatusUI() ?? {
  icon: null,
  title: "",
  message: "",
};

  return (
    <Transition show={showProfileUpdateModal} as={Fragment}>
      <Dialog
        onClose={() => dispatch(setShowProfileUpdateModal(false))}
        className="relative z-[9999]"
      >
        <TransitionChild
          as={Fragment}
          enter="ease-out duration-300"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-200"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm" />
        </TransitionChild>

        <div className="fixed inset-0 flex items-center justify-center p-4">
          <TransitionChild
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0 scale-90"
            enterTo="opacity-100 scale-100"
            leave="ease-in duration-200"
            leaveFrom="opacity-100 scale-100"
            leaveTo="opacity-0 scale-90"
          >
            <Dialog.Panel className="w-full max-w-md rounded-2xl bg-[var(--wrapperColor)] p-6 text-center shadow-xl transition-all">
              {icon && <div className="flex justify-center mb-4">{icon}</div>}

              <Dialog.Title
                as="h3"
                className="text-xl font-semibold text-[var(--textColor)]"
              >
                {title}
              </Dialog.Title>
              <p className="mt-2 text-[var(--textColor)]/70">{message}</p>

              {updateStatus === "loading" && (
                <div className="mt-6 w-full">
                  <div className="h-3 w-full bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-blue-600 transition-all duration-300 ease-in-out"
                      style={{ width: `${progress}%` }}
                    />
                  </div>
                  <p className="text-sm text-[var(--textColor)]/60 mt-1">
                    {progress}%
                  </p>
                </div>
              )}

              {updateStatus !== "loading" && (
                <div className="mt-6">
                  <button
                    onClick={() => dispatch(setShowProfileUpdateModal(false))}
                    className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-12 py-2 text-white shadow-md transition hover:bg-blue-700 active:scale-95"
                  >
                    Close
                  </button>
                </div>
              )}
            </Dialog.Panel>
          </TransitionChild>
        </div>
      </Dialog>
    </Transition>
  );
}
