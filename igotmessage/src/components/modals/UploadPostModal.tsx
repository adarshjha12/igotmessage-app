...
import { setShowPostUploadModal } from "@/features/postSlice";

export default function UploadModal() {
  const dispatch = useAppDispatch();
  const uploadStatus = useAppSelector((state) => state.post.uploadPostStatus);
    ...
    switch (uploadStatus) {
      case "loading":
        return {
          ...
          message: "Hold tight! Your Post is uploading.",
        };
      case "succeeded":
        return {
...
const { icon, title, message } = getStatusUI() ?? {
  ...
  return (
    <Transition show={showPostUploadModal} as={Fragment}>
      <Dialog
        onClose={() => dispatch(setShowPostUploadModal(false))}
          ...
          >
            <Dialog.Panel className="w-full max-w-md rounded-2xl bg-[var(--wrapperColor)] p-6 text-center shadow-xl transition-all">
              ...
              {uploadStatus !== "loading" && (
                <div className="mt-6">
                  <button
                    onClick={() => dispatch(setShowPostUploadModal(false))}
