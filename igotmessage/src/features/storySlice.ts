...
interface StoryState {
  ...
  uploadStoryStatus: "idle" | "loading" | "succeeded" | "failed";
...
export const handleStoryUpload = createAsyncThunk<Res, UploadArgs>(
  "story/uploadStory",
  async ({ userId, file, musicData }) => {
    ...
    const response = await axios.post<Res>(
      `${backendUrl}/api/story/upload-story`,
      formData,
      {
        headers: {
