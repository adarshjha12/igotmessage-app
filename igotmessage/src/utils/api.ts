...
const fetchMyStories = async function (userId: string) {
  const url =
    process.env.NODE_ENV === "production"
      ? `${process.env.NEXT_PUBLIC_PRODUCTION_BACKEND_URL}/api/story/get-my-stories`
      : `${process.env.NEXT_PUBLIC_LOCAL_BACKEND_URL}/api/story/get-my-stories`;
  try {
...
const fetchOtherStories = async function () {
  const url =
    process.env.NODE_ENV === "production"
      ? `${process.env.NEXT_PUBLIC_PRODUCTION_BACKEND_URL}/api/story/get-other-stories`
      : `${process.env.NEXT_PUBLIC_LOCAL_BACKEND_URL}/api/story/get-other-stories`;
...
const handleGuest = async () => {
  try {
    const url =
      process.env.NODE_ENV === "production"
        ? `${process.env.NEXT_PUBLIC_PRODUCTION_BACKEND_URL}/api/guest/guest-auth`
        : `${process.env.NEXT_PUBLIC_LOCAL_BACKEND_URL}/api/guest/guest-auth`;
